...
----
For shearing transformation the off-diagonal terms need to be modified. For the ease of visualization, I kept the off-diagonal element to either $1$ or $-1$.
...
---
Orthogonal Matrices only perform rotation transformation to vectors with any scaling. There is extensive explanation for this type of matrices in the later sections, hence, I will just provide one simple example of orthogonal matrix.
$$
\begin{bmatrix}
\cos(\theta) & -\sin(\theta) \\
\sin(\theta) & \cos(\theta)
\end{bmatrix}
$$
By varying the value of $\theta$, we can generate orthogonal matrices which provide $\theta$ amount of rotations.
