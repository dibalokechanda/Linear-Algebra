...
---
Orthogonal Matrices only perform rotation transformation to vectors without any scaling. There is extensive explanation for this type of matrices in the later sections, hence, I will just provide one simple example of orthogonal matrix.
$$
\begin{bmatrix}
\cos(\theta) & -\sin(\theta) \\
\sin(\theta) & \cos(\theta)
