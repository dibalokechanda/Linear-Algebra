...
$$
- For all, $\mathbf{v_1}, \mathbf{v_2}\in \mathbb{W}$; $\mathbf{v_1}+\mathbf{v_2} \in \mathbb{W}$. This means **the subspace is closed under addition operation**. In other words, if we pick any two vectors from the subspace, the resultant vector from the addition of those two vectors is also in the subspace.
