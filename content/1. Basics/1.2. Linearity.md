...
The visualization shown above is an affine transformation in $\mathbb{R}^2$.  Given a vector $\mathbf{x}$ in $\mathbb{R}^2$, $\mathbf{A} \mathbf{x}$  is a linear transformation. But when an additional shift $\mathbf{b}$ is applied it is no longer linear. 
