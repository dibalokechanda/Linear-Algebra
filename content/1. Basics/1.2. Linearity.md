...
![[Pasted image 20231228124942.png]]

The visualization shown above is an affine transformation in $\mathbb{R}^2$.  Given a vector $\mathbf{x}$ in $\mathbb{R}^2$, $\mathbf{A} \mathbf{x}$  is a linear transformation, where $\mathbf{A}\in \mathbb{R}^{2\times 2}$. But when an additional shift $\mathbf{b}\in \mathbb{R}^2$ is applied it is no longer linear. 
