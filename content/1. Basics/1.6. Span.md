...
Spanning set is a specific set of vectors from a vector space. When we take the linear combination of those vectors it spans the entire vectors space. For example in $\mathbb{R}^3$,  one example of spanning set is $\{ \mathbf{e_{1}},\mathbf{e_{2}},\mathbf{e_{3}}\}$. 
